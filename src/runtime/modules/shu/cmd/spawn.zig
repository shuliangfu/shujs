...
/// Shu.cmd.spawn(options)：异步，返回 Promise<{ status, stdout, stderr }>；纯 Zig 实现，无内联 JS
fn spawnCallback(
    ...
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.spawn requires --allow-run" }) catch {};
        return jsc.JSValueMakeUndefined(ctx);
    }
    if (argumentCount == 0) return jsc.JSValueMakeUndefined(ctx);
    const options_js = arguments[0];
    return async_mod.createRunPromise(ctx, options_js);
}
...
fn spawnSyncCallback(
    ...
    const opts = globals.current_run_options orelse return jsc.JSValueMakeUndefined(ctx);
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.spawnSync requires --allow-run" }) catch {};
        return jsc.JSValueMakeUndefined(ctx);
    }
    if (argumentCount == 0) return jsc.JSValueMakeUndefined(ctx);
