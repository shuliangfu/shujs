...
/// Shu.cmd.exec(cmd [, options])：异步，返回 Promise<{ stdout, stderr, code }>；纯 Zig 实现，无内联 JS
fn execCallback(
    ...
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.exec requires --allow-run" }) catch {};
        return jsc.JSValueMakeUndefined(ctx);
    }
    if (argumentCount == 0) return jsc.JSValueMakeUndefined(ctx);
    const cmd_js = arguments[0];
    return async_mod.createExecPromise(ctx, cmd_js);
}

/// Shu.cmd.execSync(cmd [, options])：同步，返回 { stdout, stderr, code }；需 --allow-run
fn execSyncCallback(
    ctx: jsc.JSContextRef,
    _: jsc.JSObjectRef,
    _: jsc.JSObjectRef,
    ...
    const opts = globals.current_run_options orelse return jsc.JSValueMakeUndefined(ctx);
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.execSync requires --allow-run" }) catch {};
        return jsc.JSValueMakeUndefined(ctx);
    }
    if (argumentCount == 0) return jsc.JSValueMakeUndefined(ctx);
