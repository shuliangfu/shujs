...
pub fn register(ctx: jsc.JSGlobalContextRef, shu_obj: jsc.JSObjectRef, allocator: ?std.mem.Allocator) void {
    ...
    const name_cmd = jsc.JSStringCreateWithUTF8CString("cmd");
    defer jsc.JSStringRelease(name_cmd);
    const a = allocator orelse @import("../../../globals.zig").current_allocator orelse return;
    const cmd_val = getExports(ctx, a);
    _ = jsc.JSObjectSetProperty(ctx, shu_obj, name_cmd, cmd_val, jsc.kJSPropertyAttributeNone, null);
}
