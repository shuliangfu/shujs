...
/// Shu.cmd.run(options)：异步，返回 Promise<{ status, stdout?, stderr? }>；纯 Zig 实现，无内联 JS
fn runCallback(
    ...
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.run requires --allow-run" }) catch {};
...
fn runSyncCallback(
    ...
    const opts = globals.current_run_options orelse return jsc.JSValueMakeUndefined(ctx);
    if (!opts.permissions.allow_run) {
        errors.reportToStderr(.{ .code = .permission_denied, .message = "Shu.cmd.runSync requires --allow-run" }) catch {};
        return jsc.JSValueMakeUndefined(ctx);
    }
    if (argumentCount == 0) return jsc.JSValueMakeUndefined(ctx);
