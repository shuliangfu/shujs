...
| node:errors                 | shu:errors              | ✅ 已实现  | SystemError、codes（ERR\_\* 与常见系统错误码）与 node:errors 对齐                                                                        |
...
| **dirname / **filename                                                 | 当前模块目录/路径 | ✅        | 引擎/bindings 注入                                                    |
...
- **已覆盖**：绝大部分 node: 内置、全局 process/Buffer/console/timers/require/**dirname/**filename/fetch/WebSocket/crypto/URL 等，**已可运行多数 Node 项目**。
