...
/// 判断值是否为布尔类型
pub extern "c" fn JSValueIsBoolean(JSContextRef, JSValueRef) bool;
/// 判断值是否为数字类型
pub extern "c" fn JSValueIsNumber(JSContextRef, JSValueRef) bool;
/// 判断值是否为字符串（用于 Web Crypto algorithm 参数等）
pub extern "c" fn JSValueIsString(JSContextRef, JSValueRef) bool;
/// 判断值是否为对象类型（函数/数组/普通对象等）
pub extern "c" fn JSValueIsObject(JSContextRef, JSValueRef) bool;
