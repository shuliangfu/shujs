...
// -----------------------------------------------------------------------------
// 异步文件 I/O（独立 io_uring，仅 READ/WRITE，与 HighPerfIO 网络环分离）
// -----------------------------------------------------------------------------
const FILE_RING_ENTRIES: u16 = 256;
const MAX_FILE_PENDING: usize = 256;
const FILE_USER_DATA_TAG: u64 = 1 << 61; // 与 RECV/SEND 区分，file 完成时 user_data 高位置位
...
/// 异步文件 I/O 句柄：独立 io_uring 仅做 pread/pwrite，供 fs 层 submitReadFile/submitWriteFile，pollCompletions 返回 tag=file_read/file_write
pub const AsyncFileIO = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !AsyncFileIO {
        var ring = linux.IoUring.init(FILE_RING_ENTRIES, 0) catch return error.SystemOutdated;
        errdefer ring.deinit();
    ...
    /// 提交异步读；buffer_ptr[0..len] 为读入目标，完成时 Completion.buffer_ptr/len 有效
    ...
    /// 提交异步写；data_ptr[0..len] 在完成前须保持有效
    ...
    /// 收割文件 I/O 完成项；返回切片有效至下次 pollCompletions 前
    ...
    inline fn popFree(self: *AsyncFileIO) ?usize {
        if (self.free_len == 0) return null;
        self.free_len -= 1;
        return self.free_stack[self.free_len];
    }
    inline fn pushFree(self: *AsyncFileIO, slot: usize) void {
