...
// -----------------------------------------------------------------------------
// 异步文件 I/O（工作线程 + ReadFile/WriteFile 阻塞调用，与 HighPerfIO 的 IOCP 分离）
// 远期可改为同一 IOCP + overlapped ReadFile/WriteFile 以统一事件源
// -----------------------------------------------------------------------------
...
pub const AsyncFileIO = struct {
    ...
    job_queue: std.ArrayList(FileJobWin) = undefined,
    ...
    pub fn init(allocator: std.mem.Allocator) !AsyncFileIO {
        var job_queue = std.ArrayList(FileJobWin).initCapacity(allocator, MAX_FILE_PENDING_WIN) catch return error.OutOfMemory;
        const done_list = std.ArrayList(api.Completion).initCapacity(allocator, MAX_FILE_PENDING_WIN) catch {
    ...
    fn workerRunWin(self: *AsyncFileIO) void {
        while (!self.shutdown.load(.acquire)) {
            self.job_mutex.lock();
            const job: ?FileJobWin = if (self.job_queue.items.len > 0)
            ...
            if (job) |j| {
                ...
                if (j.op == .read) {
                    ...
                    if (kernel32.SetFilePointerEx(j.handle, move, null, FILE_BEGIN) == 0) {
    ...
    pub fn submitReadFile(
        ...
        if (self.job_queue.items.len >= MAX_FILE_PENDING_WIN) return error.TooManyPending;
