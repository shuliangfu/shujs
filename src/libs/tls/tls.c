...
tls_client_ctx_t *tls_client_ctx_create(const char *ca_path, int verify_peer) {
    ...
    /* ALPN: 客户端通告 h2 与 http/1.1，握手后可用 tls_get_alpn_selected 判断是否走 HTTP/2 */
    static const unsigned char alpn_client[] = { 2, 'h', '2', 8, 'h', 't', 't', 'p', '/', '1', '.', '1' };
    if (SSL_CTX_set_alpn_protos(ctx, alpn_client, sizeof(alpn_client)) != 0) {
        SSL_CTX_free(ctx);
        return NULL;
    }
