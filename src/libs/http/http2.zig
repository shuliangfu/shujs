// HTTP/2 协议层（libs）：帧解析（SETTINGS/HEADERS/DATA 等）、HPACK、preface；供 server（h2/h2c）与 http2_client 共用
...
/// HPACK 编码请求头（客户端用）：:method、:path、:scheme、:authority 字面量不索引（0x00 + name + value）；顺序符合 RFC 7540 8.1.2.3。返回写入字节数。
pub fn encodeRequestHeaders(out: []u8, method: []const u8, path: []const u8, scheme: []const u8, authority: []const u8) !usize {
    const names_values = [_]struct { name: []const u8, value: []const u8 }{
    ...
    var pos: usize = 0;
    for (names_values) |nv| {
        if (nv.name.len > 127 or nv.value.len > 127) return error.BufferTooSmall;
        const need = 1 + 1 + nv.name.len + 1 + nv.value.len;
        if (pos + need > out.len) return error.BufferTooSmall;
        out[pos] = 0x00; // literal without indexing
        pos += 1;
        out[pos] = @intCast(nv.name.len);
        pos += 1;
        @memcpy(out[pos..][0..nv.name.len], nv.name);
        pos += nv.name.len;
        out[pos] = @intCast(nv.value.len);
        pos += 1;
        @memcpy(out[pos..][0..nv.value.len], nv.value);
        pos += nv.value.len;
    }
    return pos;
}
...
// 单元测试已迁至 src/tests/runtime/modules/shu/server/http2.zig（路径导入 libs/http/http2.zig）
