...
//! - **HTTP/2（自动）**：对 **https** URL 自动先做 TLS 连接并 ALPN 协商；若协商结果为 `h2` 则走 HTTP/2 路径（preface、SETTINGS、HEADERS/DATA），否则回退到 HTTP/1.1。无需调用方传参；HTTP/2 路径复用项目内 server 的帧解析与 HPACK 解码。
...
//! - `error.Http2NotAvailable`：https 时 ALPN 未协商为 h2 或未启用 TLS，已回退到 HTTP/1.1（内部使用，不向调用方返回）。
...
//! ## 调试
//!
//! 设置 **SHU_DEBUG_HTTP=1** 时，向 stderr 打印：https 请求使用的协议（**h2** 或 **http/1.1 fallback**）、空 body、ReadFailed、ResponseTooLarge 等调试信息。
//! 若大量请求显示 http/1.1 fallback，多为**服务端未在 TLS ALPN 中通告 h2**（如 registry.npmmirror.com、npm.jsr.io 等常见 registry 当前多仅支持 HTTP/1.1）。
//!
//! ## 示例
//!
...
const http2_client = @import("http2_client");
...
/// HTTP/2 响应类型，与 http2_client 一致；供 shu:http2 等使用。调用方 free body 与可分配的 status_text。
pub const H2Response = http2_client.H2Response;
/// 未启用 TLS 或 ALPN 非 h2 时由 requestH2 返回；供 shu:http2 判断回退。
pub const Http2NotAvailable = http2_client.Http2NotAvailable;
...
/// [Allocates] 仅 https：直接走 HTTP/2（ALPN h2），返回 H2Response。调用方 free body 与可分配的 status_text。
/// 若 ALPN 非 h2 或连接失败则返回 error.Http2NotAvailable，供 shu:http2 等回退或提示。
pub fn requestH2(allocator: std.mem.Allocator, url: []const u8, options: RequestOptions) !H2Response {
    return http2_client.requestViaH2(allocator, url, options);
}
...
/// 当 SHU_DEBUG_HTTP 非空时，向 stderr 打印本次 https 请求使用的协议（h2 或 http/1.1 fallback）。
/// fallback 表示 TLS ALPN 协商结果非 h2（多为服务端未在 ALPN 中通告 h2，如 npmmirror / npm.jsr.io）。
fn debugLogProtocol(url: []const u8, protocol: []const u8) void {
    const env = std.c.getenv("SHU_DEBUG_HTTP") orelse return;
    if (std.mem.span(env).len == 0) return;
    const io = libs_process.getProcessIo() orelse return;
    var buf: [256]u8 = undefined;
    var w = std.Io.File.Writer.init(std.Io.File.stderr(), io, &buf);
    w.interface.print("\n[shu http] {s} url={s}\n", .{ protocol, url }) catch return;
    w.flush() catch return;
}
...
/// 当 url 为 https 时自动先尝试 HTTP/2（ALPN h2），成功则直接返回 body，否则回退到当前 client 的 HTTP/1.1。
pub fn getWithClient(client: *std.http.Client, allocator: std.mem.Allocator, url: []const u8, options: GetOptions) ![]const u8 {
    ...
    if (std.mem.eql(u8, uri.scheme, "https")) {
        ...
        if (http2_client.requestViaH2(allocator, url, opts)) |h2r| {
            debugLogProtocol(url, "h2");
            if (h2r.status >= 200 and h2r.status < 300) {
                if (h2r.status_text_is_allocated) allocator.free(h2r.status_text);
                return h2r.body;
            }
            allocator.free(h2r.body);
            if (h2r.status_text_is_allocated) allocator.free(h2r.status_text);
            return error.BadStatus;
        } else |e| {
            if (!std.mem.eql(u8, @errorName(e), "Http2NotAvailable")) return e;
            debugLogProtocol(url, "http/1.1 fallback");
        }
    }
    var headers: [16]Header = undefined;
    var n: usize = 0;
...
fn requestViaZig(allocator: std.mem.Allocator, url: []const u8, options: RequestOptions) !Response {
    const uri = std.Uri.parse(url) catch return error.InvalidUrl;
    if (std.mem.eql(u8, uri.scheme, "https")) {
        if (http2_client.requestViaH2(allocator, url, options)) |h2r| {
            debugLogProtocol(url, "h2");
            return .{
                .status = h2r.status,
                .status_text = h2r.status_text,
                .status_text_is_allocated = h2r.status_text_is_allocated,
                .body = h2r.body,
                .content_encoding = h2r.content_encoding,
            };
        } else |e| {
            if (!std.mem.eql(u8, @errorName(e), "Http2NotAvailable")) return e;
            debugLogProtocol(url, "http/1.1 fallback");
