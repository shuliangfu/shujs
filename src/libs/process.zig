...
/// 从环境变量或 CPU 数得到基准并发上限（内部共用）；按 CPU 的 2 倍计算，适应 I/O 密集型并发；返回 [1, max]。
fn concurrencyCapBase(max: u32) u32 {
    ...
    const n: u32 = @intCast(@min(n_usize, std.math.maxInt(u32)));
    // 2× CPU 作为基准，I/O 密集型时并发可高于核数；避免 n*2 溢出
    const twice: u32 = if (n <= std.math.maxInt(u32) / 2) n * 2 else std.math.maxInt(u32);
    var cap_val = @min(max, twice);
    if (cap_val == 0) cap_val = 1;
    return cap_val;
}

/// 用于**并发下载**（网络 + 落盘）的并发上限；基准为 CPU 数的 2 倍，忙时压低。
///
/// **环境变量（可选）**：SHU_CONCURRENCY_CAP 直接指定并发上限（u32），解析成功则 clamped 到 1..max 并直接返回。
///
/// **逻辑**：1）基准 = min(max, 2×CPU)；2）若运行时探测到内存紧张 / **磁盘忙** / 网络忙，
...
/// 用于**纯并发网络请求**（如 API、解析请求）的并发上限；基准为 CPU 数的 2 倍，忙时压低；不考虑磁盘 I/O。
///
/// **环境变量（可选）**：SHU_CONCURRENCY_CAP 同上。
///
/// **逻辑**：1）基准 = min(max, 2×CPU)；2）若运行时探测到**内存紧张或网络忙**（不查磁盘），
