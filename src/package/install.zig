...
/// npm 解析 worker：每 worker 持有一个 Client，循环取任务并调用 resolveVersionTarballAndDeps，结果写入 results[i]。完成一个即更新 resolving_progress（与 Installing worker 更新 install_completed_count 一致），供 CLI 轮询。
